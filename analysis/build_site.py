...
/* Fill follows the TP slot alongside the shape, so two series landing on the
   same point stay two series: solid, a paler tone of the same hue, hollow, a
   deeper one. k0 is the plain solid every chart drew before this existed.
   k2 is see-through rather than surface-filled -- an opaque hollow marker still
   erases what it covers, which is the whole problem -- with pointer-events
   pinned so its tooltip survives having no paint in the middle.
   color-mix is progressive enhancement: where it is unsupported the whole
   declaration drops and the marker falls back to the solid fill above, leaving
   shape as the encoding it already was. */
.dot.k1 {{ fill:color-mix(in oklab, var(--c) 45%, var(--bg)); }}
.dot.k2 {{ fill:transparent; stroke:var(--c); pointer-events:all; }}
.dot.k3 {{ fill:color-mix(in oklab, var(--c) 72%, var(--fg)); }}
.bar {{ fill:var(--c); }}
